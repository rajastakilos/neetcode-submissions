    ...
    public void islandsAndTreasure(int[][] grid) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int[] dir : DIRS) {
                ...
                if (x < 0 || x >= grid.length || y < 0 || y >= grid[0].length) continue; // boundaries
                if (grid[x][y] == -1) continue; // water
                if (grid[x][y] == 0) continue; // treasure
                if (grid[x][y] != UNVISITED_EMPTY_ROOM) continue;
            
                
                q.add(new int[]{x, y}); // New node (coordinate)
                grid[x][y] = grid[curr[0]][curr[1]] + 1; // Mark visited and Assign distance.
            }
        }
    }
}
// Every cell is assigned its distance exactly once, the first time it is reached.
