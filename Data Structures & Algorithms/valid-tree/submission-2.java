class Solution {
    public boolean validTree(int n, int[][] edges) {
        if (edges.length > n - 1) return false; // 1. A tree must have exactly n - 1 edges. Necessary condition.

        List<List<Integer>> adj = new ArrayList<>();
        for (int i = 0; i < n; i++) adj.add(new ArrayList<>());

        for (int[] edge : edges) { // 2. Build Undirected graph. 2 way connection. List of pairs
        ...
        q.offer(new int[]{0, -1}); // 3. 0 is arbitrary. -1 is a dummy value since root node has no parent.
        visit.add(0); // The moment we add the node we mark it visited. 

        while (!q.isEmpty()) { // 4. BFS
            int[] pair = q.poll();
            int node = pair[0];
            int parent = pair[1];
