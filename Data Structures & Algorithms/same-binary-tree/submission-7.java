...
class Solution {
    public boolean isSameTree(TreeNode p, TreeNode q) {
        ...
        while (!pNodeQueue.isEmpty() && !qNodeQueue.isEmpty()) {
            ...
            // Check Values of current nodes from p and q.
            if (pNode.val != qNode.val) return false;

            // Check shapes of children of p and q.
            if (pNode.right != null && qNode.right == null) return false;
            if (pNode.right == null && qNode.right != null) return false;
            if (pNode.left != null && qNode.left == null) return false;
            if (pNode.left == null && qNode.left != null) return false;

            // Enqueue children of p and q.
...
// Time: O(min(p + q)) where m is the number of nodes in p and q is the number of nodes in q.
    // Worst case is when both trees are same size then its (O(p + q)).
